...
class TiffColorizer(tk.Tk):
    def __init__(self):
        ...
        self.tiff_path = None
        self.base_rgb = None       # original uint8, never modified
        self.base_display = None   # inverted copy if invert is on, else same as base_rgb
    ...
    def _build_ui(self):
        ...
        tk.Checkbutton(right, text="Invert Colors", variable=self.invert,
                       command=self._on_invert_toggle).pack(anchor="w")
    ...
    def _open_tiff(self):
        ...
        try:
            ...
            self.base_rgb = arr
            self.base_display = np.array(ImageOps.invert(Image.fromarray(arr))) if self.invert.get() else arr
            self.tiff_path = path
    ...
    def _save_tiff(self):
        ...
        try:
            ...
            base_u8 = to_uint8(base_arr)
            if invert:
                base_u8 = np.array(ImageOps.invert(Image.fromarray(base_u8)))
            adj_pil = apply_adjustments(base_u8, r_gain, g_gain, b_gain,
    ...
    def _on_invert_toggle(self):
        if self.base_rgb is None:
            return
        if self.invert.get():
            self.base_display = np.array(ImageOps.invert(Image.fromarray(self.base_rgb)))
        else:
            self.base_display = self.base_rgb
        self._schedule_update()
    ...
    def _update_preview(self):
        if self.base_rgb is None or self.base_display is None:
            return
